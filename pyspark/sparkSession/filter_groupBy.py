...
people_df.select("firstName", year("birthDate")).filter(
    "gender='M'" and year("birthDate") > "1960"
)

people_df.groupBy("gender").count().show()
people_df.orderBy("firstName").groupBy("firstName").count().show(5)
people_df.groupBy("firstName").count().orderBy("count", ascending=False).show(
    5
)  # count()로 sort
people_df.groupBy("firstName").count().orderBy("count").show(5)

people_df.groupBy("firstName").count().filter(col("count") > "5430").orderBy(
...
dordon_df = (
    people_df.select(year("birth_date").alias("birth_year"), "first_name")
    .filter((col("first_name") == "Donna") | (col("first_name") == "Dorothy"))  # or
    .filter("gender == 'F'")
    .orderBy("birth_year")
    .groupBy("birth_year", "first_name")  # Multiple Group By
    .count()
    .show(5)
)
